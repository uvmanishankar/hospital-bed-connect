...
function NurseUpdateDrawer({
  ...
  const mutation = useMutation({
    mutationFn: (input: { sysId: string; conditionNotes: string }) =>
      updateConditionNotes({
        data: { ...input, nurseSubmittedBy: user?.sys_id },
      }),
    onSuccess: (result) => {
    ...
    onError: (err: unknown) => {
      // Show ServiceNow's real error (e.g. mandatory field, ACL, invalid field
      // name) instead of a generic message, so failures are diagnosable.
      const message = err instanceof Error ? err.message : "Unknown error from ServiceNow.";
      toast.error("Save failed", {
        description: message,
        duration: 10000,
      });
      // eslint-disable-next-line no-console
      console.error("[updateConditionNotes] failed:", message);
    },
  });
