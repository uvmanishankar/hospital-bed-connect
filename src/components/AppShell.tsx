...
import {
  LayoutDashboard, Bed, DoorOpen, ClipboardList, LogIn, LogOut,
  Stethoscope, Boxes, FileText, Users, Bell, Settings, HelpCircle,
  Search, Menu, ShieldCheck, ChevronDown, ListChecks
} from "lucide-react";
import { useState, type ReactNode } from "react";
import { Logo } from "./Logo";
import { logout, useAuth } from "@/lib/auth-store";

interface NavItem { to: string; label: string; icon: typeof Bed; badge?: number }
...
export function AppShell({ children, title, breadcrumb, actions }: {
  children: ReactNode; title: string; breadcrumb?: string[]; actions?: ReactNode;
}) {
  const { user } = useAuth();
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Sidebar */}
      <aside
        className={`fixed lg:static z-40 inset-y-0 left-0 w-72 bg-sidebar text-sidebar-foreground flex flex-col transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
          ...
          {navItems.map((it) => {
            ...
            return (
              ...
              >
                ...
                {it.badge && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${active ? "bg-white/20" : "bg-primary text-primary-foreground"}`}>
                    {it.badge}
                  </span>
                )}
              </Link>
        ...
        <div className="p-3 border-t border-sidebar-border space-y-3">
          <div className="rounded-xl bg-sidebar-accent/60 p-3 flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10px] uppercase tracking-wider text-sidebar-foreground/60">Current Role</div>
          ...
          <div className="flex items-center gap-2.5 px-1">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-sm font-semibold truncate">{user?.name ?? "Dr. Priya Sharma"}</div>
              <div className="text-[11px] text-sidebar-foreground/60 truncate">{user?.role ?? "Ward Manager"}</div>
            </div>
          </div>
          <button
          ...
          <div className="flex items-center gap-3 px-4 lg:px-6 h-16">
            ...
            <div className="flex items-center gap-2.5 pl-2 sm:pl-3 sm:border-l border-border">
              ...
              <div className="hidden sm:block leading-tight">
                ...
                <div className="text-[11px] text-muted-foreground">{user?.role ?? "Ward Manager"}</div>
              </div>
        ...
        <div className="px-4 lg:px-6 pt-6 pb-2">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4">
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-secondary truncate">{title}</h1>
              {breadcrumb && (
...
export function StatCard({ label, value, sublabel, icon: Icon, tone = "primary" }: {
  label: string; value: ReactNode; sublabel?: ReactNode; icon: typeof Bed;
